...
export const loadImage = (url) => new Promise((imageLoaded, imageError) => {
  // Support importing assets that may be ESM module objects (e.g. { default: '/_next/static/..' })
  const resolved = (url && typeof url === 'object' && 'default' in url) ? url.default : url;
  const image = new Image();
  image.onload = () => imageLoaded(image);
  image.onerror = () => imageError(new Error(`Failed to load image: ${resolved}`));
  image.src = resolved;
});
